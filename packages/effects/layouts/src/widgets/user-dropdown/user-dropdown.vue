...
<template>
  ...
  <DropdownMenu v-model:open="openPopover" :modal="false">
    <DropdownMenuTrigger ref="refTrigger" :disabled="props.trigger === 'hover'">
      <div
        class="mr-2 ml-1 cursor-pointer rounded-md p-1.5 transition-colors hover:bg-accent"
      >
        <div class="flex-center hover:text-accent-foreground">
          <VbenAvatar
            ...
            dot
          />
          <span v-else class="px-1 text-sm font-medium text-foreground">
            {{ text }}
          </span>
          <ChevronDown class="ml-1 size-4 text-muted-foreground" />
        </div>
      </div>
    </DropdownMenuTrigger>
